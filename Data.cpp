...
{///name.find("BP_Rifle_AKM_Wrapper_C") != std::string::npos
	...
	if (Setting::deadbox) if (Name.find("PlayerDeadBox_C") != string::npos)return "Dead-Box";
...
VOID Data::QueryThread()
{
	...
	DWORD GWorld = Utility::ReadMemoryEx<UINT>(Data::ViewMatrixBase + 114572) + 60;
	DWORD GName = Utility::ReadMemoryEx<UINT>(Data::ViewMatrixBase - 3117016) + 136;
	DWORD UWorld = 0;
	...
	while (true)
	{
		...
		for (int i = 0; i < EntityCount; ++i)
		{
			...
			if (IsPlayer(EntityType))
			{
				...
				AActor.IsDead = Utility::ReadMemoryEx<bool>(EntityAddress + Offset::bDead);
