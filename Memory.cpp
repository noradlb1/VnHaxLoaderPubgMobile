...
VECTOR2 GetMagicCoordinates()
{
	...
	/*Normal*/
	DWORD SceneComponent = Utility::ReadMemoryEx<INT>(Offset::RootComponent + 0x140);// 0x14c);//SceneComponent* HomingTargetComponent
	DWORD VehicleCommon = Utility::ReadMemoryEx<INT>(Offset::VehicleCommon + 0x6f8);// 0x6dc);//VehicleCommonComponent* VehicleCommon

	/*Danger*/
	//DWORD SceneComponent = Utility::ReadMemoryEx<INT>(Offset::RootComponent + 0x140);//SceneComponent* HomingTargetComponent? old 14c
	//DWORD VehicleCommon = Utility::ReadMemoryEx<INT>(Offset::VehicleCommon + 0x698);//VehicleCommonComponent* VehicleCommon? old 6a0
...
int Memory::AimFindBestTarget() {
	...
	{
		DWORD tmpAddv = Utility::ReadMemoryEx<INT>(Data::AActorList[i].Address + 832);
		DWORD bodyAddv = tmpAddv + 320;
		DWORD boneAddv = Utility::ReadMemoryEx<INT>(tmpAddv + 1504) + 48;
		VECTOR2 chest;
		VECTOR2 head;
	...
	while (true)
	{
		if (Data::NetDriver > 0)
		{
			if (Setting::MagicBullet || Setting::engine == 1)
				//if (Setting::MagicBullet || Setting::MagicBullet == 1)
			{
		...
		////IPad View
		//bool IPadViewChanged = false;

		else
		{
			/*DWORD Camera = Utility::ReadMemoryEx<DWORD>(Data::LocalPlayer + 0x1348) + 0x260;
			if (Setting::IPADSIZE1 != 0)
			{
				Utility::WriteMemoryEx<float>(Camera, Setting::IPADSIZE1);
			}*/
		}

		////FPS + UlTraHD //Offset Update and working
		bool FPS = false;


		//Fast Run //Offset Update and working
		if (Setting::fastrun)
		{
			if (GetAsyncKeyState(VK_LSHIFT) & 1) {
				...
				Utility::WriteMemoryEx<float>(Data::LocalPlayer + 0x2280, write);
			}
			else if (GetAsyncKeyState(VK_LSHIFT) == 0)
			{
				float writee = 1.0f;

				Utility::WriteMemoryEx<float>(Data::LocalPlayer + 0x2280, writee);
			}
		}
		...
		//		auto currentvehicla = Utility::ReadMemoryEx<DWORD>(Utility::ReadMemoryEx<DWORD>(Utility::ReadMemoryEx<DWORD>(Data::LocalPlayer + 0xfd0) + 0x194) + 0x58);//0x138) + 0x140) + 0x24);
		//		if (currentvehicla)
		//		{
		...
		////Les Recoil //Offset Update and working
		if (Setting::recoil)
		{
			////V1
			...
			////V2
			int weaponOffSet = Utility::ReadMemoryEx<INT>(Data::LocalPlayer + 0x1e9c);
			if (weaponOffSet != 0)
			{
				int shootOffset = Utility::ReadMemoryEx<INT>(weaponOffSet + 0xbc0);

				if (shootOffset != 0)
				{
					Utility::WriteMemoryEx<float>(shootOffset + 0x860, 0);
					Utility::WriteMemoryEx<float>(shootOffset + 0x864, 0);
					Utility::WriteMemoryEx<float>(shootOffset + 0x868, 0);
		...
		//	DWORD body = Utility::ReadMemoryEx<INT>(Data::LocalPlayer + 0x11f0);
		//	DWORD camera = Utility::ReadMemoryEx<INT>(Data::LocalPlayer + 0x1344);
		//	if (GetAsyncKeyState('Y'))// (TOP view) key Y
		//	{
		//		FLOAT ground;
		//		ReadProcessMemory(Game::hProcess, (LPCVOID)root, &ground, sizeof(ground), nullptr);
		//		while (true)
		//		{
		//			Utility::WriteMemoryEx<FLOAT>(root, ground + 309);
		...
		//		}
		//	}
		//	//if (GetAsyncKeyState(0x0042)) // B (bottom view)
		//	//{
		//	//	Utility::WriteMemoryEx<FLOAT>(camera + 0x120, -288);
		//	//	Utility::WriteMemoryEx<FLOAT>(body + 0x120, -288);
		//	//	godview = true;
		//	//}
		//	//if (GetAsyncKeyState(0x0046)) // F (front view)
		//	//{
		//	//	Utility::WriteMemoryEx<FLOAT>(camera + 0x118, 288);
		//	//	Utility::WriteMemoryEx<FLOAT>(body + 0x118, 288);
		//	//	godview = true;
		//	//}
		//	//if (GetAsyncKeyState(0x0051)) // Q  (left view)
		//	//{
		//	//	Utility::WriteMemoryEx<FLOAT>(camera + 0x11C, -370);
		//	//	Utility::WriteMemoryEx<FLOAT>(body + 0x11C, -370);
		//	//	godview = true;
		//	//}
		//	//if (GetAsyncKeyState(0x0045)) // E (right view)
		//	//{
		//	//	Utility::WriteMemoryEx<FLOAT>(camera + 0x11C, 370);
		//	//	Utility::WriteMemoryEx<FLOAT>(body + 0x11C, 370);
		//	//	godview = true;
		...
		//	DWORD body = Utility::ReadMemoryEx<INT>(Data::LocalPlayer + 0x1344);// 0x11d0);
		//	DWORD camera = Utility::ReadMemoryEx<INT>(Data::LocalPlayer + 0x11f0);// 0x1324);
		...
		//	if (Setting::GodView)
		//	{
		//		DWORD root = Utility::ReadMemoryEx<INT>(Data::LocalPlayer + 0x16c) + 0x164;// 0x158;
...
////Allinone
		...
		/*if (Setting::ipadview)
		{
			DWORD Camera = Utility::ReadMemoryEx<DWORD>(Data::LocalPlayer + 0x1328) + 0x260;
			if (Setting::IPADSIZE != 0)
			{
				Utility::WriteMemoryEx<float>(Camera, Setting::IPADSIZE);
			}
		}
		else
		{
